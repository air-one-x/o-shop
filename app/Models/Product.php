...
class Product extends CoreModel {
    ...
    public static function insert($objet){
        ...
        $req = $bdd->prepare('INSERT INTO `product`(`name`,`description`,`picture`,`price`,`brand_id`,`category_id`,`type_id`)VALUES(?,?,?,?,?,?,?)');
        $req->execute(array(
            $objet->getName(),
    ...
    public static function update($objet){
        $bdd= new PDO('mysql:host=localhost;dbname=oshop','oshop','oshop',array(PDO::ATTR_ERRMODE=>PDO::ERRMODE_WARNING));
        $req = $bdd->prepare('UPDATE `product` SET `name`=?, `description`=?, `picture`=?, `price`=?, `brand_id`=?, `category_id`=?, `type_id`=? WHERE `id`=?');
        $req->execute(array(
            $objet->getName(),
            $objet->getDescription(),
            $objet->getPicture(),
